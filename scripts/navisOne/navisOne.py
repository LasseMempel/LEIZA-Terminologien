...
from rdflib.namespace import SKOS, RDF, VANN, DCTERMS
...
g.add((thesaurus, DCTERMS.title, Literal("NAVISone", lang="de")))


g.add((thesaurus, DCTERMS.description, Literal("NAVISone ist ein Thesaurus über Schiffsbegriffe", lang="de")))

g.add((thesaurus, DCTERMS.creator, Literal("Florian Thiery")))
...
for index, row in parentDf.iterrows():
    ...
    for index2, row2 in df[df["fk_id_parent"] == row["id"]].iterrows():
        ...
        if not pd.isnull(row2["origindesc"]):
            ...
            g.add((concept2, DCTERMS.source, Literal(row2["origindesc"])))
